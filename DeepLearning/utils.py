...
def save_checkpoint(filepath, G_AB, G_BA, D_A, D_B, optimizerG, optimizerD, epoch, is_best=False):
    ...
    * 체크포인트 저장
    ...
    # state 정보 담기
    state = {
        ConstVar.KEY_STATE_G_AB: G_AB.state_dict(),
        ConstVar.KEY_STATE_G_BA: G_BA.state_dict(),
        ConstVar.KEY_STATE_D_A: D_A.state_dict(),
        ConstVar.KEY_STATE_D_B: D_B.state_dict(),
...
def save_pics(pics_list, filepath, title):
    ...
    * 학습 결과물 이미지로 저장
    ...
    plt_pics_list = [(
        ...
        fake_a.cpu().detach().reshape(-1, 256, 256).permute(1, 2, 0),
        fake_b.cpu().detach().reshape(-1, 256, 256).permute(1, 2, 0),
        rec_a.cpu().detach().reshape(-1, 256, 256).permute(1, 2, 0),
        rec_b.cpu().detach().reshape(-1, 256, 256).permute(1, 2, 0)
    ) for a, b, fake_a, fake_b, rec_a, rec_b in pics_list]

    # plt 에 그리기
    fig, axs = plt.subplots(nrows=ConstVar.NUM_PICS_LIST, ncols=6, figsize=(10, 15))
    ...
    for num, (a, b, fake_a, fake_b, rec_a, rec_b) in enumerate(plt_pics_list):
        ...
        axs[num, 1].axis('off')
        axs[num, 2].imshow(X=fake_a * 0.5 + 0.5, cmap='gray')
        axs[num, 2].axis('off')
        axs[num, 3].imshow(X=fake_b * 0.5 + 0.5, cmap='gray')
        axs[num, 3].axis('off')
        axs[num, 4].imshow(X=rec_a * 0.5 + 0.5, cmap='gray')
        axs[num, 4].axis('off')
        axs[num, 5].imshow(X=rec_b * 0.5 + 0.5, cmap='gray')
        axs[num, 5].axis('off')
...
class ReplayBuffer:
    def __init__(self, replay_buffer_max_size=50):
        ...
        * replay buffer 관리 클래스
        ...
        assert replay_buffer_max_size > 0, print(ConstVar.MSG_REPLAY_BUFFER_ERROR)
